use crate::module_view::module_widget::{GanttChartData, MIN_CHART_HEIGHT, MIN_CHART_WIDTH, ModuleWidget, RESIZE_HANDLE_SIZE, WidgetTpye};
...
impl GanttChartRenderer {
    ...
    pub fn draw_gantt_chart(
        ...
        let min_start = chart_data.data_points.iter().map(|t| t.start_time).fold(f32::MAX, f32::min);
        let max_end = chart_data.data_points.iter().map(|t| t.end_time).fold(0.0f32, f32::max);
        ...
        let time_center = min_start + time_range * 0.5 + module_widget.settings.x_offset * time_range;
        ...
        let y_scroll_offset = (module_widget.settings.y_offset * total_content_height).clamp(0.0, max_y_offset * total_content_height);
        ...
        for (idx, task) in chart_data.data_points.iter().enumerate() {
            ...
            if y_pos + bar_height < area.y + padding + 30.0 || y_pos > area.y + area.height - padding {
                continue;
            }
            ...
            let start_ratio = if time_max_visible - time_min_visible > 0.0 {
                ((task.start_time - time_min_visible) / (time_max_visible - time_min_visible)).clamp(0.0, 1.0)
            } else {
                0.0
            };
            
            let end_ratio = if time_max_visible - time_min_visible > 0.0 {
                ((task.end_time - time_min_visible) / (time_max_visible - time_min_visible)).clamp(0.0, 1.0)
            } else {
                1.0
            };
            ...
            let bar_x_end = (chart_x_start + end_ratio * chart_width).min(chart_x_start + chart_width);
            ...
            if bar_width > 0.0 {
                ...
                // Draw bar border
                let bar_rect = canvas::Path::rectangle(bar_position, Size::new(bar_width.max(2.0), scaled_bar_height));
                frame.stroke(
                    &bar_rect,
                    canvas::Stroke::default()
